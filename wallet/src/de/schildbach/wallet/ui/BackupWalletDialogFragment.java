        ...
        if (requestCode == REQUEST_CODE_CREATE_DOCUMENT) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                try {
                    ...
                    cipherOut = new OutputStreamWriter(activity.getContentResolver().openOutputStream(uri),
                            Charsets.UTF_8);
                    cipherOut.write(Crypto.encrypt(plainBytes, password.toCharArray()));
                    cipherOut.flush();

                    log.info("backed up wallet to: '" + uri + "'");
                    final DialogBuilder dialog = new DialogBuilder(activity);
                    dialog.setTitle(R.string.export_keys_dialog_title);
                    dialog.setMessage(Html.fromHtml(getString(R.string.export_keys_dialog_success, uri)));
